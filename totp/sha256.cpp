...
struct sha256_context : sha256_base
{
    ...
    void update(const util::byte* message, unsigned int len);
    void final(digest_type& digest);

    void transform(const util::byte* message, unsigned int block_nb);
    unsigned int m_tot_len = 0;
    unsigned int m_len = 0;
    util::byte m_block[2 * SHA224_256_BLOCK_SIZE]{};
    std::uint32_t m_h[8]
    {
...
{                                             \
    *((str) + 3) = (util::byte) ((x)      );       \
    *((str) + 2) = (util::byte) ((x) >>  8);       \
    *((str) + 1) = (util::byte) ((x) >> 16);       \
    *((str) + 0) = (util::byte) ((x) >> 24);       \
}
...
void sha256_context::update(const util::byte* message, unsigned int len)
{
    unsigned int block_nb;
    unsigned int new_len, rem_len, tmp_len;
    const util::byte* shifted_message;
...
{
    sha256_base::digest_type digest{};
